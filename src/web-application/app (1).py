...
@app.route('/download_private_key/media/public-keys/<filename_pvt>')
def download_private_key(filename_pvt):
    filename = UPLOAD_KEY+filename_pvt
    return send_file(filename, as_attachment=True)
...
def register_user():
    ...
    while True:
        filename_pvt = UPLOAD_KEY+username+'p-PrivateKey.pem'
        if not os.path.exists(filename_pvt):
            break
    
    with open(filename_pvt, "wb") as f:
